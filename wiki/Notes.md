...
* The OneWoW pin button on the world map, next to the map filters. It sits under the other addon icons already in that stack.
...
On the **Zones** tab, pin the zone (**Pin Zone Window**) to show a floating window when you enter. **Zone Pinned Window** on that tab (and in Notes settings) turns every floating zone window on or off.

Hover the window for **Show Zone Notes** and **Hide Scrollbar**. Uncheck Show Zone Notes to leave only the pin list when that list is docked. **Hide Scrollbar** hides the pin list bar; the list still scrolls with the mouse wheel. Opacity on the note, hover bar, and pin list stay in step.
...
OneWay Pins owns the list. **Show pin list in zone** (OneWay Pins settings, on by default) shows a floating list when you enter a zone that has pins marked **Show on pinned window**. No marked pins, no list.

Each pin and each pack has **Show on pinned window**, **Show on World Map**, and **Show on Minimap** (all on by default, including shipped packs). Turn **Show on pinned window** off on a huge treasure pack so the map still has the pins but the list stays usable.

If a zone note is also pinned, the list docks beside it. The list hides in instances, Delves, pet battles, and battlegrounds by default (OneWay Pins settings). Character, bank, guild bank, and professions windows draw in front of the list.

**Add** on the pin list (or right-click empty space) opens **Add Here** and **Find Location**. Left-click a listed pin to go, Ctrl-click to open the OneWay Pins tab, right-click for Go / Track, Show on Map, Edit, Open Pin Tab, or Delete. Creating a pin does not create a zone note.
...
* **Pin Packs** group many pins behind one row (for example moths or trainers). The pack stays one row even if it has dozens of pins. Turn a pack off to hide every pin in it. Each pack also has **Show on pinned window**, **Show on World Map**, and **Show on Minimap**. **Show Disabled Packs** stays on so you can turn a pack back on; uncheck it to hide those rows.
...
* Leave **Show on pinned window** on for the pins you want in the zone list. Turn it off on a huge pack so you still see those pins on the map without filling the list. Uncheck **Show Zone Notes** if you only want that list. Check **Hide Scrollbar** if you want that list without the bar.
