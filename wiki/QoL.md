...
Built-in with QoL (not separate toggles): toast-style alerts (New Collections when you loot something you do not have yet, Upgrade Alerts when a gear upgrade hits your bags, instance and note alerts) and the Portals hub / Travel card. Mages get separate Teleport and Portal flyouts on ESC; show or hide each set in Portals settings. ESC portal icons use short destination names; Portals settings can enlarge that text or hide it so you use the icon and tooltip only. You can pick a specific hearthstone toy, show only this season's Hero's Path flyouts, let new dungeon teleports come from the game, and turn on a Group Finder teleport prompt after you join a dungeon.
