...
QoL has a **Toggles** tab (not a module). It is a searchable list of current game Options: gameplay, interface, nameplates, combat text, camera, chat, audio, and graphics. Star a row to keep it at the top. Changes write the same settings as Esc > Options.
...
QoL has a **Waypoints** tab (not a module). **Waypoint arrow** chooses where a click sends a coordinate: the Blizzard map pin, or TomTom when that addon is enabled. TomTom shows Not Detected until it is loaded, and cannot be turned on then. One row is Enabled. Turning TomTom off uses the Blizzard map pin again. The same choice covers tracker steps, OneWay Pins, vendors, NPCs, and hearth. The arrow name is the step, pin, vendor, NPC, or place you clicked.
