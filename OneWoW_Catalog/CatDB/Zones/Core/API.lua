...
local placeByZoneMapID
local placesByInstanceMapID
...
---@param dest table<number, string[]>
---@param mapID number|nil
---@param placeKey string
local function AddPlaceMapKey(dest, mapID, placeKey)
    if not mapID or mapID <= 0 then
        return
    end
    local list = dest[mapID]
    if not list then
        list = {}
        dest[mapID] = list
    end
    for i = 1, #list do
        if list[i] == placeKey then
            return
        end
    end
    tinsert(list, placeKey)
end
    ...
    for placeKey, place in pairs(ns.Places) do
        ...
        if kind == "zone" then
            ...
            if mapID and mapID > 0 then
                placeByZoneMapID[mapID] = placeKey
            end
        else
            AddPlaceMapKey(placesByInstanceMapID, place.mapID, placeKey)
            AddPlaceMapKey(placesByInstanceMapID, place.uiMapID, placeKey)
        end
    end
    -- Multi-floor instances (Skyreach 601/602) store extra uiMaps on encounters.
    for _, enc in pairs(ns.Encounters) do
        local instanceID = enc.instanceID
        local ui = enc.uiMapID
        if instanceID and instanceID > 0 and ui and ui > 0 then
            local placeKey = placeByInstanceID[instanceID]
            if placeKey then
                AddPlaceMapKey(placesByInstanceMapID, ui, placeKey)
            end
        end
    end
end
...
--- Zone / city card for a UiMap ID or zone mapID. Does not hydrate encounters.
---@param expansionID number|nil
---@param mapID number
---@return table|nil place
function OneWoW_CatDB_ZoneDB_API.GetZoneInstance(expansionID, mapID)
    ...
    if expansionID then
        return GetCard(placeKey, expansionID)
    end
    return GetCard(placeKey)
end

--- All instance cards for a world map ID. Does not hydrate encounters.
---@param mapID number
---@return table places
function OneWoW_CatDB_ZoneDB_API.GetInstancesByMapID(mapID)
    ...
    EnsurePlaceLookups()
    local keys = placesByInstanceMapID[mapID]
    if not keys then
        return out
    end
    for k = 1, #keys do
        local placeKey = keys[k]
        local place = ns.Places[placeKey]
        if place then
            local expansions = ListingExpansions(placeKey, place)
            for i = 1, #expansions do
...
local COLLECTIBLE_COUNT_TYPES = {
    TMog = true, Mount = true, Pet = true, Toy = true, Recipe = true, Quest = true, Housing = true,
}

--- Collectible totals for a place from shipped loot rows. Does not hydrate
--- encounter cards or load ItemDB. Types stay 0 until ItemDB is already loaded.
---@param inst table|nil
---@return table<string, { current: number, total: number }>
function OneWoW_CatDB_ZoneDB_API.CountPlaceCollectibles(inst)
    local counts = {
        TMog = { current = 0, total = 0 },
        Mount = { current = 0, total = 0 },
        Pet = { current = 0, total = 0 },
        Toy = { current = 0, total = 0 },
        Recipe = { current = 0, total = 0 },
        Quest = { current = 0, total = 0 },
        Housing = { current = 0, total = 0 },
    }
    local ids = inst and inst.encounterIDs
    if not ids then
        return counts
    end
    local seen = {}
    for i = 1, #ids do
        local enc = ns.Encounters[ids[i]]
        local loot = enc and enc.loot
        if loot then
            for j = 1, #loot do
                local itemID = loot[j].itemID
                if itemID and not seen[itemID] then
                    seen[itemID] = true
                    local special, itemData = ItemSpecial(itemID, loot[j])
                    local row = special and COLLECTIBLE_COUNT_TYPES[special] and counts[special]
                    if row then
                        row.total = row.total + 1
                        if OneWoW_CatDB_ZoneDB_API.IsItemCollected(itemID, itemData, special) then
...
function OneWoW_CatDB_ZoneDB_API.InvalidateDataLookups()
    wipe(cardsByKey)
    dropIndex = nil
    dropNameIndex = nil
    ...
    placeByUiMapID = nil
    placeByZoneMapID = nil
    placesByInstanceMapID = nil
end
...
function OneWoW_CatDB_ZoneDB_API.EnsureEncounters(inst)
    ...
    -- Catalog journal hydrate may load Items. ESC, AFK, toasts, and Item Tracker
    -- must not call this just to show the current zone.
